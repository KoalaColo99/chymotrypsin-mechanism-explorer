(() => {
  "use strict";
  const BUILD_ID = "2026.07.23-r10";
  const stageCheckpoints = [
    "Asp102 organizes His57; His57 Nε2 faces the Ser195 O–H proton.",
    "The aromatic P1 group binds while the scissile carbonyl is positioned beside Ser195.",
    "GENERAL BASE: His57 Nε2 accepts the Ser195 proton, producing the Ser195 alkoxide.",
    "The Ser195 alkoxide attacks carbonyl C; C=O π electrons move to substrate oxygen.",
    "TI1: substrate-derived O⁻ is stabilized by Gly193 and Ser195 backbone N–H donors.",
    "GENERAL ACID: His57 protonates the leaving amine as the peptide C–N bond breaks.",
    "Acyl-enzyme pause: one product has left; the acyl fragment remains ester-linked to Ser195.",
    "GENERAL BASE: His57 removes a water proton, creating the water-derived nucleophile.",
    "Water-derived oxygen attacks the acyl carbonyl; C=O becomes C–O⁻.",
    "TI2: the second substrate-derived oxyanion is stabilized by the same oxyanion hole.",
    "GENERAL ACID: His57 returns H⁺ to Ser195 as the Ser–O–acyl bond breaks.",
    "Products leave; Ser195–OH and the catalytic triad are regenerated."
  ];
  ...
  const structureModes = {
    "focus-active-site": { residues: window.ACTIVE_SITE_RESIDUES, status: "Viewing active site: residues within the catalytic pocket", minRadius: 4, extraRadius: 2.5 },
    "catalytic-triad": { residues: [{name:"HIS",number:57,chainGroup:"hisAsp"},{name:"ASP",number:102,chainGroup:"hisAsp"},{name:"SER",number:195,chainGroup:"serPocket"}], status: "Viewing catalytic triad: Asp102, His57, Ser195", minRadius: 3.5, extraRadius: 2 },
    "oxyanion-hole": { residues: [{name:"GLY",number:193,chainGroup:"serPocket"},{name:"SER",number:195,chainGroup:"serPocket"}], status: "Viewing oxyanion hole: Gly193 and Ser195 backbone", minRadius: 3, extraRadius: 1.5 },
    "specificity-pocket": { residues: [{name:"SER",number:189,chainGroup:"serPocket"},{name:"GLY",number:216,chainGroup:"serPocket"},{name:"GLY",number:226,chainGroup:"serPocket"}], status: "Viewing S1 specificity pocket: Ser189, Gly216, Gly226", minRadius: 4, extraRadius: 2 }
  };
  ...
  async function applyStructureMode(mode, buttonId) {
    ...
    try {
      ...
      await state.viewer.structureInteractivity({ elements, action: "focus", focusOptions: { minRadius: definition.minRadius, extraRadius: definition.extraRadius, durationMs: 500 } });
    } catch (error) {
  ...
  async function focusActiveSite() {
    ...
    try {
      if (elements) {
        await state.viewer.structureInteractivity({ elements, action: "focus", focusOptions: { minRadius: 4, extraRadius: 2.5, durationMs: 450 } });
      } else {
  ...
  function drawMechanismLegacy(stage) {
  ...
  const chemistryTemplateLibrary = (() => {
    const bond = (x1,y1,x2,y2,cls="") => `<path class="bond ${cls}" d="M${x1} ${y1}L${x2} ${y2}"/>`;
    const doubleBond = (x1,y1,x2,y2,cls="") => {
      const dx=x2-x1,dy=y2-y1,length=Math.hypot(dx,dy)||1,ux=dx/length,uy=dy/length;
      const separation=5.5,shorten=15,px=-uy*separation,py=ux*separation;
      const ax=x1+ux*shorten,ay=y1+uy*shorten,bx=x2-ux*shorten,by=y2-uy*shorten;
      return `<g class="canonical-double-bond ${cls}"><path d="M${(ax+px).toFixed(1)} ${(ay+py).toFixed(1)}L${(bx+px).toFixed(1)} ${(by+py).toFixed(1)}"/><path d="M${(ax-px).toFixed(1)} ${(ay-py).toFixed(1)}L${(bx-px).toFixed(1)} ${(by-py).toFixed(1)}"/></g>`;
    };
    const atom = (key,x,y,symbol,charge="",atomId="",info="") => `<g class="atom-node template-atom" data-atom="${key}" data-info="${escapeXml(info)}" transform="translate(${x} ${y})"><circle class="atom-hit" r="19"/><text class="atom-symbol" text-anchor="middle" y="7">${symbol}</text>${charge?`<text class="formal-charge" x="13" y="-12">${charge}</text>`:""}${atomId?`<text class="atom-id" x="0" y="27" text-anchor="middle">${atomId}</text>`:""}</g>`;
    const pairs = (owner,x,y,angles,count) => angles.slice(0,count).map(angle=>`<g class="lone-pair template-pair" data-owner="${owner}" transform="translate(${x} ${y}) rotate(${angle})"><circle cx="10" cy="-2.6" r="2.1"/><circle cx="10" cy="2.6" r="2.1"/></g>`).join("");
    const residue = (name,x,y) => `<text class="template-residue" x="${x}" y="${y}">${name}</text>`;
    /*
     * Canonical P1 peptide coordinates. These positions are deliberately fixed
     * across substrate, TI1, acyl enzyme, water-bound, TI2, and product states.
     * The reaction center stays compact while the nonreacting P1 substituent
     * extends into its own upper-right region.
     */
    const P1 = Object.freeze({
      carbonylC:[825,250], carbonylO:[825,160], alphaC:[900,294],
      leavingN:[770,300], serAttach:[732,250], waterAttach:[854,313]
    });
    const aromaticP1 = () => `<g class="template aromatic-p1">${bond(917,284,936,250,"p1-sidechain")}${bond(936,250,962,211,"p1-sidechain")}${bond(934,136,962,120)}${bond(962,120,990,136)}${bond(990,136,990,168)}${bond(990,168,962,184)}${bond(962,184,934,168)}${bond(934,168,934,136)}<path class="ring-inner" d="M941 139L961 128M983 139L983 163M960 176L941 166"/>${bond(962,211,962,184,"p1-sidechain")}<text class="template-residue p1-title" x="900" y="98">P1 aromatic side chain</text></g>`;
    const asp102 = () => `<g class="template asp-template">${residue("Asp102",118,174)}${bond(120,250,163,250)}${bond(163,250,198,222)}${doubleBond(198,222,239,190,"asp-carbonyl")}${bond(198,222,241,255)}${atom("aspOD1",251,181,"O","","OD1","Asp102 OD1|Oxygen|0|2|2|Carboxylate oxygen")}${pairs("aspOD1",251,181,[-70,20],2)}${atom("aspOD2",253,263,"O","−","OD2","Asp102 OD2|Oxygen|−1|1|3|Hydrogen-bond acceptor")}${pairs("aspOD2",253,263,[-50,20,90],3)}</g>`;
    const his57 = ({protonated=false}={}) => `<g class="template his-template">${residue("His57",390,174)}${bond(392,240,420,207)}${bond(420,207,461,218)}${bond(461,218,470,260)}${bond(470,260,433,282)}${bond(433,282,399,260)}${bond(399,260,392,240)}<path class="ring-inner" d="M422 216L451 224M461 255L438 270"/>${atom("hisND1",394,240,"N","","Nδ1","His57 Nδ1|Nitrogen|0|3|1|Faces Asp102")}${atom("hisN",470,260,"N",protonated?"+":"","Nε2",`His57 Nε2|Nitrogen|${protonated?"+1":"0"}|${protonated?"3":"2"}|${protonated?"0":"1"}|Faces Ser195 and shuttles protons`)}${protonated?"":pairs("hisN",470,260,[0],1)}${bond(391,229,378,207)}<text class="hydrogen-label" x="368" y="201">H</text></g>`;
    const ser195 = ({alkoxide=false}={}) => `<g class="template ser-template">${residue("Ser195",620,174)}${bond(620,250,692,250)}${atom("serO",716,250,"O",alkoxide?"−":"","Oγ",`Ser195 Oγ|Oxygen|${alkoxide?"−1":"0"}|${alkoxide?"1":"2"}|${alkoxide?"3":"2"}|Primary nucleophile`)}${pairs("serO",716,250,[-60,55,180],alkoxide?3:2)}</g>`;
    const peptideBackbone = ({leaving=false,includeAlpha=true}={}) => `<g class="template peptide-backbone">${includeAlpha?bond(P1.alphaC[0]-17,P1.alphaC[1]-10,P1.carbonylC[0]+17,P1.carbonylC[1]+9,"alpha-carbonyl"):""}${leaving?"":`${bond(P1.carbonylC[0]-17,P1.carbonylC[1]+11,P1.leavingN[0]+17,P1.leavingN[1]-12,"scissile")}${atom("peptideN",P1.leavingN[0],P1.leavingN[1],"N","","N","Peptide nitrogen|Nitrogen|0|3|1|Scissile peptide nitrogen")}${pairs("peptideN",P1.leavingN[0],P1.leavingN[1],[155],1)}${bond(P1.leavingN[0]-17,P1.leavingN[1]+10,725,327,"peptide-continuation")}${bond(725,327,690,312,"peptide-continuation")}`}</g>`;
    const peptideCarbonyl = ({reduced=false,charge="",leaving=false,product=false}={}) => `<g class="template carbonyl-template">${atom("alphaC",P1.alphaC[0],P1.alphaC[1],"C","","Cα","P1 alpha carbon|Carbon|0|4|0|Peptide backbone")}${atom("carbonylC",P1.carbonylC[0],P1.carbonylC[1],"C","","C′",`Carbonyl carbon|Carbon|0|${reduced?"4":"3"}|0|${reduced?"Tetrahedral center":"Planar carbonyl"}`)}${reduced?bond(P1.carbonylC[0],P1.carbonylC[1]-19,P1.carbonylO[0],P1.carbonylO[1]+19,"carbonyl-single"):doubleBond(P1.carbonylC[0],P1.carbonylC[1]-19,P1.carbonylO[0],P1.carbonylO[1]+19,"peptide-carbonyl")}${atom("carbonylO",P1.carbonylO[0],P1.carbonylO[1],"O",charge,"O",`Carbonyl oxygen|Oxygen|${charge||"0"}|${reduced?"1":"2"}|${reduced?"3":"2"}|${reduced?"Oxyanion":"Carbonyl oxygen"}`)}${pairs("carbonylO",P1.carbonylO[0],P1.carbonylO[1],[-120,-60,0],reduced?3:2)}${product?`${bond(P1.carbonylC[0]-17,P1.carbonylC[1]+11,P1.leavingN[0]+18,P1.leavingN[1]-12)}${atom("productOH",P1.leavingN[0],P1.leavingN[1],"O","","OH","Carboxyl product hydroxyl|Oxygen|0|2|2|Hydrolysis product")}${pairs("productOH",P1.leavingN[0],P1.leavingN[1],[135,-135],2)}${bond(P1.leavingN[0]-13,P1.leavingN[1]+13,742,328)}${atom("productH",729,337,"H","","","Product proton|Hydrogen|0|1|0|Carboxylic acid proton")}`:peptideBackbone({leaving})}</g>`;
    const tetrahedralCarbon = ({second=false,leaving=false}={}) => `<g class="template tetrahedral-template ${second?"ti2":"ti1"}">${atom("alphaC",P1.alphaC[0],P1.alphaC[1],"C","","Cα","P1 alpha carbon|Carbon|0|4|0|Peptide backbone")}${atom("carbonylC",P1.carbonylC[0],P1.carbonylC[1],"C","","C*","Tetrahedral carbon|Carbon|0|4|0|sp3 approximately 109.5 degrees")}${bond(P1.carbonylC[0],P1.carbonylC[1]-19,P1.carbonylO[0],P1.carbonylO[1]+19,"tetrahedral-bond")}${atom("carbonylO",P1.carbonylO[0],P1.carbonylO[1],"O","−","O","Oxyanion|Oxygen|−1|1|3|Oxyanion-hole acceptor")}${pairs("carbonylO",P1.carbonylO[0],P1.carbonylO[1],[-120,-60,0],3)}${bond(P1.carbonylC[0]-17,P1.carbonylC[1]+4,P1.serAttach[0],P1.serAttach[1],"tetrahedral-bond")}${bond(P1.carbonylC[0]+15,P1.carbonylC[1]+12,P1.alphaC[0]-16,P1.alphaC[1]-10,"tetrahedral-bond")}${second?`${bond(P1.carbonylC[0]+13,P1.carbonylC[1]+14,P1.waterAttach[0]-14,P1.waterAttach[1]-12,"tetrahedral-bond")}${atom("waterO",P1.waterAttach[0],P1.waterAttach[1],"O","","Ow","Water-derived oxygen|Oxygen|0|2|2|Hydrolytic oxygen")}${pairs("waterO",P1.waterAttach[0],P1.waterAttach[1],[-35,55],2)}`:""}${peptideBackbone({leaving,includeAlpha:false})}</g>`;
    const water = ({activated=false}={}) => `<g class="template water-template">${atom("waterO",720,350,"O",activated?"−":"","Ow",`Water oxygen|Oxygen|${activated?"−1":"0"}|${activated?"1":"2"}|${activated?"3":"2"}|Hydrolytic nucleophile`)}${pairs("waterO",720,350,[-120,-60,60],activated?3:2)}${bond(704,358,685,372)}${atom("waterH1",672,382,"H","","","Water hydrogen|Hydrogen|0|1|0|Water proton")}${activated?"":`${bond(736,358,755,372)}${atom("waterH2",768,382,"H","","","Water hydrogen|Hydrogen|0|1|0|Water proton")}`}</g>`;
    return { bond,doubleBond,atom,pairs,aromaticP1,asp102,his57,ser195,peptideCarbonyl,tetrahedralCarbon,water,P1 };
  })();

  function drawMechanism(stage) {
    const T=chemistryTemplateLibrary,mode=stage.bonds;
    const bound=mode!=='resting',tetra=['tetrahedral','tetrahedral2'].includes(mode);
    const hisPlus=['activation','attack','tetrahedral','water','waterattack','tetrahedral2'].includes(mode);
    const alkoxide=['activation','attack'].includes(mode),waterVisible=['water','waterattack','deacyl'].includes(mode);
    const leaving=['acyl','water','waterattack','tetrahedral2','deacyl'].includes(mode);
    const arrows=[];
    if(mode==='activation')arrows.push(`<path class="electron-arrow" d="M482 255Q555 222 647 248"/><path class="electron-arrow" d="M681 242Q700 224 710 238"/>`);
    if(mode==='attack')arrows.push(`<path class="electron-arrow" d="M724 239Q770 214 809 245"/><path class="electron-arrow" d="M835 230Q856 204 830 178"/>`);
    if(mode==='collapse')arrows.push(`<path class="electron-arrow" d="M833 179Q852 211 833 239"/><path class="electron-arrow" d="M808 263Q786 282 773 287"/><path class="electron-arrow proton-transfer-arrow" d="M526 252Q640 319 747 301"/>`);
    if(mode==='water')arrows.push(`<path class="electron-arrow" d="M481 266Q555 333 650 370"/><path class="electron-arrow" d="M685 370Q703 353 713 341"/>`);
    if(mode==='waterattack')arrows.push(`<path class="electron-arrow" d="M728 338Q773 305 810 266"/><path class="electron-arrow" d="M834 230Q855 204 830 178"/>`);
    if(mode==='deacyl')arrows.push(`<path class="electron-arrow" d="M833 179Q852 212 833 239"/><path class="electron-arrow" d="M808 263Q770 276 742 258"/><path class="electron-arrow proton-transfer-arrow" d="M526 252Q610 215 690 246"/>`);
    let reaction='';
    if(bound){
      reaction=T.aromaticP1();
      if(mode==='tetrahedral')reaction+=T.tetrahedralCarbon({second:false,leaving:false});
      else if(mode==='tetrahedral2')reaction+=T.tetrahedralCarbon({second:true,leaving:true});
      else reaction+=T.peptideCarbonyl({reduced:['attack','waterattack'].includes(mode),charge:['attack','waterattack'].includes(mode)?'−':'',leaving,product:mode==='release'});
    }
    const covalent=['attack','collapse','acyl','water','waterattack','deacyl'].includes(mode);
    const covalentBond=covalent?T.bond(T.P1.serAttach[0],T.P1.serAttach[1],T.P1.carbonylC[0]-18,T.P1.carbonylC[1],`covalent ${mode==='attack'?'forming':''}`):'';
    const esterAnnotation=mode==='acyl'?`<g class="ester-annotation"><path class="ester-leader" d="M760 352Q775 314 781 265"/><text x="650" y="370">Acyl-enzyme ester bond</text></g>`:'';
    const hBonds=`<g class="template-hbonds"><path class="hbond" d="M268 263L382 244"/><path class="hbond his-ser-contact" d="M486 260L648 250"/>${tetra?`<path class="hbond oxyanion-hbond" d="M790 112L820 145"/><path class="hbond oxyanion-hbond" d="M862 112L831 145"/><text class="donor-label" x="742" y="96">Gly193 backbone N–H</text><text class="donor-label" x="844" y="96">Ser195 backbone N–H</text>`:''}</g>`;
    const protonAtSer=['resting','bound','deacyl','release'].includes(mode),protonAtHis=['activation','attack','tetrahedral','water','waterattack','tetrahedral2'].includes(mode),protonAtLeaving=mode==='collapse';
    const protonMarkup=protonAtSer?`<g class="proton-shuttle ${mode==='deacyl'?'his-to-ser':''}" data-proton-id="H-shuttle">${T.bond(700,250,680,250,'proton-bond')}${T.atom('proton',663,250,'H','','H*','Tracked shuttle proton|Hydrogen|0|1|0|Bound to Ser195 Oγ and facing His57 Nε2')}</g>`:protonAtHis?`<g class="proton-shuttle ${['activation','water'].includes(mode)?(mode==='activation'?'ser-to-his':'water-to-his'):''}" data-proton-id="H-shuttle">${T.bond(486,260,500,260,'proton-bond')}${T.atom('proton',518,260,'H','','H*','Tracked shuttle proton|Hydrogen|+1 modeled|1|0|Bound to His57 Nε2')}</g>`:protonAtLeaving?`<g class="proton-shuttle his-to-leaving" data-proton-id="H-shuttle">${T.bond(758,286,746,272,'proton-bond')}${T.atom('proton',735,260,'H','','H*','Tracked shuttle proton|Hydrogen|0|1|0|Transferred to leaving amine')}</g>`:'';
    const roleMap={activation:['GENERAL BASE','accepts H⁺'],attack:['GENERAL BASE','activated Ser195'],collapse:['GENERAL ACID','donates H⁺'],water:['GENERAL BASE','accepts H⁺'],waterattack:['GENERAL BASE','activated water'],deacyl:['GENERAL ACID','returns H⁺ to Ser195']};
    const role=roleMap[mode];
    const roleMarkup=role?`<g class="his-role ${role[0].includes('BASE')?'base':'acid'}"><text x="430" y="322" text-anchor="middle">His57 · ${role[0]}</text><text x="430" y="339" text-anchor="middle">${role[1]}</text></g>`:'';
    $("chemistry-svg").innerHTML=`<title id="mechanismTitle">${escapeXml(stage.title)}</title><desc id="mechanismDesc">${escapeXml(stage.chemistry)}</desc><defs><marker id="arrowhead" markerWidth="8" markerHeight="6" refX="7" refY="3" orient="auto"><path d="M0 0L8 3L0 6Z"/></marker></defs><g class="canonical-mechanism">${T.asp102()}${T.his57({protonated:hisPlus})}${T.ser195({alkoxide})}${protonMarkup}${hBonds}${roleMarkup}<g class="substrate-template ${bound?'is-visible':'is-hidden'}">${reaction}${covalentBond}${esterAnnotation}</g><g class="water-state ${waterVisible?'is-visible':'is-hidden'}">${waterVisible?T.water({activated:mode==='water'}):''}</g><g class="electron-arrows-layer">${arrows.join('')}</g></g>`;
    const caption=mode==='tetrahedral'?"First tetrahedral intermediate (TI1): fixed sp³ template (≈109.5°); the oxyanion is stabilized by two backbone N–H donors.":mode==='tetrahedral2'?"Second tetrahedral intermediate (TI2): fixed sp³ template (≈109.5°); water-derived oxygen is incorporated before collapse.":stageCheckpoints[state.stage];
    $("stageCheckpoint").textContent=caption;
    ...
    const svg=$("chemistry-svg");svg.setAttribute("viewBox","90 72 980 340");svg.dataset.bonds=mode;
    svg.classList.toggle("full-electron",state.detail==='electron');svg.classList.toggle("reaction-focus",state.reactionFocus);svg.classList.toggle("tetrahedral-emphasis",tetra);
    $("oxyanionCaption").hidden=!tetra&&structureViewState.mode!=="oxyanion-hole";
    document.querySelectorAll(".atom-node").forEach(node=>node.addEventListener("click",()=>inspectAtom(node)));
    updateToggles();
  }
  ...
  function setStage(index, options = {}) {
    ...
    const phaseNumber = stage.phase === "Acylation" ? 1 : 2;
    $("stageKicker").textContent = `Step ${state.stage + 1} of ${stages.length} · Phase ${phaseNumber}: ${stage.phase}`;
